...
//$stylesheet
$res =  SQL_Query_exec("SELECT name FROM stylesheets WHERE id=$user[stylesheet] LIMIT 1") or die(mysqli_error($GLOBALS["DBconnector"]));
if (mysqli_num_rows($res) == 1){
$arr = mysqli_fetch_assoc($res);
$stylesheets = "$arr[name]";
}
...
if ($user["privacy"] != "strong" || ($CURUSER["control_panel"] == "yes")) {
	...
	<tr valign="top">
		<td align="left">
		<?php echo T_("USERNAME"); ?>: <?php echo class_user($user["username"])?> &nbsp;
        ...
        <?php echo T_("THEME_NAME"); ?>: <?php echo $stylesheets?><br />
...
<?php } ?>
		...
		<?php if ($CURUSER["edit_users"] == "yes"){ echo T_("ACCOUNT_PRIVACY_LVL").": <b>".T_($user["privacy"])."</b><br />"; }?><br />
	...
	<?php
	     ...
	     echo "<tr><td colspan='2' align='left'><b><font size=2>Operating System and Browser:</font></b><br />";
         echo htmlspecialchars($user["os"])."&nbsp;&nbsp;<img src='".$site_config['SITEURL']."/images/oss/".$user["os"].".png' width='30' height='30'>&nbsp;";?><br />
         <?php echo $browser; ?></td><br />
         <?php
	//team
	$res = SQL_Query_exec("SELECT name,image FROM teams WHERE id=$user[team] LIMIT 1");
	if (mysqli_num_rows($res) == 1) {
		$arr = mysqli_fetch_assoc($res);
		echo "<tr><td colspan='2' align='center'><b><font size=4>Team Member Of:</font></b><br />";
		echo "<center><img src='".$arr["image"]."' alt='' /><br /><center><b><font size=4>".$arr["name"]."</font></b></center><br /><br /><b><a href='teams-view.php'>".T_("TEAMS")."</a></b></td></tr>"; 
	}
	?>
<hr/>
	</table>
...
if (($CURUSER["edit_users"]=="yes") && ($CURUSER['id'] == 1 || $CURUSER["class"] > $user["class"] || $CURUSER['id'] == $user['id'])){
