...
function addData($name,$text,$uid) {
  ...
  $now = get_date_time();
    $sql = "INSERT INTO ajshoutbox (date,name,text,uid) VALUES ('".$now."','".$name."','".$text."','".$uid."')";
...
function getID($position) {
    ...
    $sql =  "SELECT * FROM ajshoutbox ORDER BY id DESC LIMIT ".$position.",1";
    $conn = getDBConnection();
    $results = SQL_Query_exec($sql, $conn);
    if (!$results || empty($results)) {
        # echo 'There was an error creating the entry';
...
function deleteEntries($id) {
    ...
    $sql =  "DELETE FROM ajshoutbox WHERE id < ".$id;
    $conn = getDBConnection();
    $results = SQL_Query_exec($sql, $conn);
    if (!$results || empty($results)) {
